#!/usr/bin/env bun
/**
 * Build script for standalone Depot Agent server.
 *
...
function showHelp(): void {
  console.log(`
Standalone server build script for Depot Agent
...
function createRootConfig(config: ServerBuildConfig): void {
  ...
  // Root package.json with workspaces (Bun resolves @depot/* through this)
  const rootPkg = {
  ...
  const rootTsconfig = {
    compilerOptions: {
      ...
      paths: {
        '@depot/server-core/*': ['./packages/server-core/src/*'],
        '@depot/shared/*': ['./packages/shared/src/*'],
        '@depot/core/*': ['./packages/core/src/*'],
        '@depot/session-tools-core/*': ['./packages/session-tools-core/src/*'],
      },
    },
  };
...
# Depot Agent Server — convenience entry point
...
[Unit]
Description=Depot Agent Server
...
async function main(): Promise<void> {
  ...
  console.log(`=== Building Depot Agent Server ${version} for ${platform}-${arch} ===`);
