...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="Depot-Agent-x64.AppImage"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    if pgrep -x "Depot Agents" >/dev/null 2>&1; then
        ...
        if pgrep -x "Depot Agents" >/dev/null 2>&1; then
            ...
            pkill -9 -x "Depot Agents" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  Depot Agents has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'Depot Agents'${NC}"
    echo ""
    ...
    if pgrep -f "Depot-Agent.*AppImage" >/dev/null 2>&1; then
        info "Stopping Depot Agents..."
        pkill -f "Depot-Agent.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/Depot-Agent-x64.AppImage"
