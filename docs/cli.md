...
Terminal client for Depot Agent server. Connects over WebSocket (`ws://` or `wss://`) to a running headless server.
...
- For all other commands: a running Depot Agent headless server with URL and token
