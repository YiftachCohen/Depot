...
export async function resolveAuthEnvVars(
  ...
  } else if (authType === 'iam_credentials') {
    // IAM credentials for Bedrock — fetch stored credentials and set AWS env vars
    const iamCreds = await credentialManager.getLlmIamCredentials(connectionSlug);
    if (iamCreds) {
      envVars.AWS_ACCESS_KEY_ID = iamCreds.accessKeyId;
      envVars.AWS_SECRET_ACCESS_KEY = iamCreds.secretAccessKey;
      if (iamCreds.sessionToken) {
        envVars.AWS_SESSION_TOKEN = iamCreds.sessionToken;
      }
      if (iamCreds.region) {
        envVars.AWS_REGION = iamCreds.region;
      }
    } else {
  ...
  // Bedrock provider: tell the SDK subprocess to route through AWS Bedrock
  if (connection.providerType === 'bedrock') {
    envVars.CLAUDE_CODE_USE_BEDROCK = '1';
    // Set AWS region from connection config if not already provided by IAM credentials
    if (connection.awsRegion && !envVars.AWS_REGION) {
      envVars.AWS_REGION = connection.awsRegion;
    }
  }
