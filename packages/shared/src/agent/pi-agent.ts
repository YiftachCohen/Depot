...
// System prompt for Depot Agent context
...
export class PiAgent extends BaseAgent {
  protected backendName = 'Depot Agents Backend';
   ...
   *
   * OAuth tokens from Depot (Claude Max, ChatGPT Plus, Copilot) are passed as
   * api_key type because they function as bearer tokens that the Pi SDK's provider
   * modules use directly. The OAuth exchange happens on the Depot side; by the time
  ...
  private handleSubprocessEvent(event: Record<string, unknown>): void {
    ...
    // Feed them through PiEventAdapter to convert to Depot AgentEvents.
   ...
   * Uses the canonical registry from @depot/session-tools-core.
   */
  ...
  protected async *chatImpl(
    ...
    try {
      ...
      const systemPrompt = getSystemPrompt(
        ...
        'Depot Agents Backend' // backendName
      );
