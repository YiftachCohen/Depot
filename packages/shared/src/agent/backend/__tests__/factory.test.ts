...
// ============================================================
// Bedrock Agent Factory Tests
// ============================================================

describe('Bedrock agent factory', () => {
  describe('providerTypeToAgentProvider for bedrock', () => {
    it('maps bedrock to anthropic (uses Anthropic SDK)', () => {
      expect(providerTypeToAgentProvider('bedrock')).toBe('anthropic');
    });
  });

  describe('Bedrock backend creation', () => {
    it('creates ClaudeAgent for bedrock provider type', () => {
      // bedrock maps to anthropic agent provider, which creates ClaudeAgent
      const provider = providerTypeToAgentProvider('bedrock');
    ...
    it('creates ClaudeAgent for bedrock with environment auth', () => {
      const provider = providerTypeToAgentProvider('bedrock');
      const config = createTestConfig({
        provider,
        providerType: 'bedrock',
    ...
    it('creates ClaudeAgent for bedrock with bearer_token auth', () => {
      const provider = providerTypeToAgentProvider('bedrock');
      const config = createTestConfig({
        provider,
        providerType: 'bedrock',
  ...
  describe('Bedrock provider-auth validation', () => {
    it('accepts iam_credentials for bedrock', () => {
      expect(isValidProviderAuthCombination('bedrock', 'iam_credentials')).toBe(true);
    });

    it('accepts environment for bedrock', () => {
      expect(isValidProviderAuthCombination('bedrock', 'environment')).toBe(true);
    });

    it('accepts bearer_token for bedrock', () => {
      expect(isValidProviderAuthCombination('bedrock', 'bearer_token')).toBe(true);
    });

    it('rejects api_key for bedrock', () => {
      expect(isValidProviderAuthCombination('bedrock', 'api_key')).toBe(false);
    });

    it('rejects oauth for bedrock', () => {
      expect(isValidProviderAuthCombination('bedrock', 'oauth')).toBe(false);
    });

    it('rejects none for bedrock', () => {
      expect(isValidProviderAuthCombination('bedrock', 'none')).toBe(false);
    });
  });

  describe('fetchBackendModels for bedrock', () => {
    it('rejects dynamic model discovery for bedrock with iam_credentials', async () => {
      const connection: LlmConnection = {
        slug: 'bedrock-iam',
        name: 'Bedrock IAM',
        providerType: 'bedrock',
    ...
    it('rejects dynamic model discovery for bedrock with environment auth', async () => {
      const connection: LlmConnection = {
        slug: 'bedrock-env',
        name: 'Bedrock Env',
        providerType: 'bedrock',
