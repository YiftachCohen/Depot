...
describe('runPreToolUseChecks', () => {
  ...
  describe('step 5: input transforms', () => {
    ...
    it('blocks direct label folder reads and suggests depot label help when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('depot label');
        expect(result.reason).toContain('depot label --help');
        expect(result.reason).toContain('labels/');
      }
    });

    it('blocks direct label config writes and suggests depot label help when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
    ...
    it('blocks direct automations config edits and suggests depot automation commands when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('depot automation');
        expect(result.reason).toContain('automations.json');
      }
    });

    it('blocks direct source config edits and suggests depot source commands when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('depot source');
        expect(result.reason).toContain('sources/linear/config.json');
      }
    });

    it('blocks direct skill file edits and suggests depot skill commands when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
      mockDetectConfigFileType.mockImplementation(() => ({
        type: 'skill',
        slug: 'commit-helper',
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('depot skill');
        expect(result.reason).toContain('skills/commit-helper/SKILL.md');
      }
    });

    it('blocks bash commands touching labels paths and points to depot label --help when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
    ...
    it('allows bash depot label commands through labels guard', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'Bash',
        input: { command: 'depot label list' },
        permissionMode: 'allow-all',
      }));
    ...
    it('blocks bash commands touching automations files and points to depot automation --help when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('depot automation --help');
        expect(result.reason).toContain('depot automation');
      }
    });

    it('allows bash depot automation commands through config-domain bash guard', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'Bash',
        input: { command: 'depot automation list' },
        permissionMode: 'allow-all',
      }));
