...
function createConsoleLogger(): Logger {
  ...
  return {
    ...
    debug: (...args) => {
      if (process.env.DEPOT_DEBUG === 'true' || process.env.DEPOT_IS_PACKAGED !== 'true') {
 ...
 * - DEPOT_APP_ROOT — override appRootPath (default: cwd)
 * - DEPOT_RESOURCES_PATH — override resourcesPath (default: cwd/resources)
 * - DEPOT_IS_PACKAGED — 'true' for production (default: false)
 * - DEPOT_VERSION — app version string (default: '0.0.0-dev')
 * - DEPOT_DEBUG — 'true' to enable debug logging
 */
export function createHeadlessPlatform(): PlatformServices {
  const logger = createConsoleLogger()
  const isDebugMode = process.env.DEPOT_DEBUG === 'true' || process.env.DEPOT_IS_PACKAGED !== 'true'

  return {
    appRootPath: process.env.DEPOT_APP_ROOT || process.cwd(),
    resourcesPath: process.env.DEPOT_RESOURCES_PATH || join(process.cwd(), 'resources'),
    isPackaged: process.env.DEPOT_IS_PACKAGED === 'true',
    appVersion: process.env.DEPOT_VERSION || '0.0.0-dev',
