...
import type { ISessionManager, IBrowserPaneManager } from '@depot/server-core/handlers'
import { createScopedLogger, CONSOLE_LOGGER, type PlatformServices, type Logger } from '@depot/server-core/runtime'
...
import { type AgentEvent, setPermissionMode, hydratePreviousPermissionMode, getPermissionModeDiagnostics, type PermissionMode, unregisterSessionScopedToolCallbacks, mergeSessionScopedToolCallbacks, AbortReason, type AuthRequest, type AuthResult, type CredentialAuthRequest, type BrowserPaneFns } from '@depot/shared/agent'
import {
  ...
  type PostInitResult,
} from '@depot/shared/agent/backend'
import { getLlmConnection, getDefaultLlmConnection, getDefaultThinkingLevel } from '@depot/shared/config'
import { PrivilegedExecutionBroker } from '@depot/server-core/services'
import { InitGate } from '@depot/server-core/domain'
import {
  getWorkspaces,
  ...
  MODEL_REGISTRY,
  type Workspace,
} from '@depot/shared/config'
import { loadWorkspaceConfig } from '@depot/shared/workspaces'
import {
...
import { toolMetadataStore, getLastApiError } from '@depot/shared/interceptor'
import { isParentTaskTool } from '@depot/shared/utils/toolNames'
import { getCredentialManager } from '@depot/shared/credentials'
import { DepotMcpClient, McpClientPool, McpPoolServer } from '@depot/shared/mcp'
...
import { evaluateAutoLabels } from '@depot/shared/labels/auto'
import { listLabels } from '@depot/shared/labels/storage'
...
import { resizeImageForAPI, resizeIconBuffer } from '@depot/server-core/services'
...
async function applyBridgeUpdates(
  ...
  mcpServers: Record<string, import('@depot/shared/agent/backend').SdkMcpServerConfig>,
  sessionId: string,
...
export class SessionManager implements ISessionManager {
  ...
  private pendingCredentialResolvers: Map<string, (response: import('@depot/shared/protocol').CredentialResponse) => void> = new Map()
  ...
  private broadcastAppThemeChanged(theme: import('@depot/shared/config').ThemeOverrides | null): void {
    if (!this.eventSink) return
  ...
  private broadcastSkillsChanged(workspaceId: string, skills: import('@depot/shared/skills').LoadedSkill[]): void {
    if (!this.eventSink) return
  ...
  async handleCredentialInput(
    sessionId: string,
    requestId: string,
    response: import('@depot/shared/protocol').CredentialResponse
  ): Promise<void> {
    const managed = this.sessions.get(sessionId)
  ...
  async createSession(workspaceId: string, options?: import('@depot/shared/protocol').CreateSessionOptions): Promise<Session> {
  ...
  async setSessionConnection(sessionId: string, connectionSlug: string): Promise<void> {
    ...
    const { getLlmConnection } = await import('@depot/shared/config/storage')
   ...
   */
  async shareToViewer(sessionId: string): Promise<import('@depot/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
    ...
    try {
      ...
      const { VIEWER_URL } = await import('@depot/shared/branding')
   ...
   */
  async updateShare(sessionId: string): Promise<import('@depot/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
      return { success: false, error: 'Session not found' }
    }
   ...
   */
  async revokeShare(sessionId: string): Promise<import('@depot/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
      return { success: false, error: 'Session not found' }
    }
  ...
  respondToPermission(
    ...
    allowed: boolean,
    alwaysAllow: boolean,
    options?: import('@depot/shared/protocol').PermissionResponseOptions,
  ): boolean {
    const managed = this.sessions.get(sessionId)
    if (managed?.agent) {
   ...
   */
  async respondToCredential(sessionId: string, requestId: string, response: import('@depot/shared/protocol').CredentialResponse): Promise<boolean> {
