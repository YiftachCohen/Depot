...
import { RPC_CHANNELS, type FileAttachment, type SendMessageOptions, type SessionEvent } from '@depot/shared/protocol'
import type { StoredAttachment } from '@depot/core/types'
import { getWorkspaceByNameOrId } from '@depot/shared/config'
import { perf } from '@depot/shared/utils'
import { isValidThinkingLevel } from '@depot/shared/agent/thinking-levels'
...
async function scanSessionDirectory(dirPath: string): Promise<import('@depot/shared/protocol').SessionFile[]> {
  const { readdir, stat } = await import('fs/promises')
  const entries = await readdir(dirPath, { withFileTypes: true })
  const files: import('@depot/shared/protocol').SessionFile[] = []
...
export const HANDLED_CHANNELS = [
  ...
  RPC_CHANNELS.sessions.SEARCH_BY_SKILL,
] as const
...
export function registerSessionsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.sessions.CREATE, async (_ctx, workspaceId: string, options?: import('@depot/shared/protocol').CreateSessionOptions) => {
  ...
  // Returns true if the response was delivered, false if agent/session is gone
  server.handle(RPC_CHANNELS.sessions.RESPOND_TO_CREDENTIAL, async (_ctx, sessionId: string, requestId: string, response: import('@depot/shared/protocol').CredentialResponse) => {
  ...
  server.handle(RPC_CHANNELS.sessions.COMMAND, async (
    _ctx,
    sessionId: string,
    command: import('@depot/shared/protocol').SessionCommand
  ) => {
    switch (command.type) {
      case 'flag':
  ...
  server.handle(RPC_CHANNELS.sessions.SEARCH_CONTENT, async (_ctx, workspaceId: string, query: string, searchId?: string) => {
    ...
    const { searchSessions } = await import('@depot/server-core/services')
    const { getWorkspaceSessionsPath } = await import('@depot/shared/workspaces')
  ...
  // ============================================================
  // Skill-scoped Session Search
  // ============================================================

  server.handle(RPC_CHANNELS.sessions.SEARCH_BY_SKILL, async (_ctx, workspaceId: string, skillSlug: string, query: string, limit?: number) => {
    log.info('[search]', 'searchBySkill:request', { workspaceId, skillSlug, query })

    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) {
      log.warn('SEARCH_BY_SKILL: Workspace not found:', workspaceId)
      return []
    }

    const { searchSessionsBySkill } = await import('@depot/server-core/services')
    const { getWorkspaceSessionsPath } = await import('@depot/shared/workspaces')

    const sessionsDir = getWorkspaceSessionsPath(workspace.rootPath)
    const results = await searchSessionsBySkill(sessionsDir, skillSlug, query, { limit })

    log.info('[search]', 'searchBySkill:response', { skillSlug, resultCount: results.length })
    return results
  })
