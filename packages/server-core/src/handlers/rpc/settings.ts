...
import { getPreferencesPath, getSessionDraft, setSessionDraft, deleteSessionDraft, getAllSessionDrafts, getWorkspaceByNameOrId, getDefaultThinkingLevel, setDefaultThinkingLevel } from '@depot/shared/config'
import { isValidThinkingLevel } from '@depot/shared/agent/thinking-levels'
import { getWorkspaceOrThrow } from '@depot/server-core/handlers'
...
export function registerSettingsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.workspace.SETTINGS_GET, async (_ctx, workspaceId: string) => {
    ...
    const { loadWorkspaceConfig } = await import('@depot/shared/workspaces')
    ...
    return {
      ...
      enabledSkillSlugs: config?.defaults?.enabledSkillSlugs,
    }
  })
  ...
  server.handle(RPC_CHANNELS.workspace.SETTINGS_UPDATE, async (_ctx, workspaceId: string, key: string, value: unknown) => {
    ...
    const validKeys = ['name', 'model', 'enabledSourceSlugs', 'enabledSkillSlugs', 'permissionMode', 'cyclablePermissionModes', 'thinkingLevel', 'workingDirectory', 'localMcpEnabled', 'defaultLlmConnection']
    ...
    const { loadWorkspaceConfig, saveWorkspaceConfig } = await import('@depot/shared/workspaces')
    const config = loadWorkspaceConfig(workspace.rootPath)
    if (!config) {
  ...
  server.handle(RPC_CHANNELS.input.GET_AUTO_CAPITALISATION, async () => {
    const { getAutoCapitalisation } = await import('@depot/shared/config/storage')
  ...
  server.handle(RPC_CHANNELS.input.SET_AUTO_CAPITALISATION, async (_ctx, enabled: boolean) => {
    const { setAutoCapitalisation } = await import('@depot/shared/config/storage')
    setAutoCapitalisation(enabled)
  })

  // Get send message key setting
  server.handle(RPC_CHANNELS.input.GET_SEND_MESSAGE_KEY, async () => {
    const { getSendMessageKey } = await import('@depot/shared/config/storage')
  ...
  server.handle(RPC_CHANNELS.power.GET_KEEP_AWAKE, async () => {
    const { getKeepAwakeWhileRunning } = await import('@depot/shared/config/storage')
  ...
  server.handle(RPC_CHANNELS.appearance.GET_RICH_TOOL_DESCRIPTIONS, async () => {
    const { getRichToolDescriptions } = await import('@depot/shared/config/storage')
  ...
  server.handle(RPC_CHANNELS.appearance.SET_RICH_TOOL_DESCRIPTIONS, async (_ctx, enabled: boolean) => {
    const { setRichToolDescriptions } = await import('@depot/shared/config/storage')
