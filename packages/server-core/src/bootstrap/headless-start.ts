...
import { ensureConfigDir, loadStoredConfig, saveConfig } from '@depot/shared/config'
...
export async function startHeadlessServer<TSessionManager, THandlerDeps>(
  ...
  const serverToken = options.serverToken ?? process.env.DEPOT_SERVER_TOKEN
  if (!serverToken) {
    throw new Error('Server token is required. Pass options.serverToken or set DEPOT_SERVER_TOKEN.')
  }

  const platform = options.platformFactory?.() ?? createHeadlessPlatform()

  const bundledAssetsRoot = options.bundledAssetsRoot
    ?? process.env.DEPOT_BUNDLED_ASSETS_ROOT
    ?? process.cwd()
  ...
  const rpcHost = options.rpcHost ?? process.env.DEPOT_RPC_HOST ?? '127.0.0.1'
  const rpcPortRaw = options.rpcPort ?? parseInt(process.env.DEPOT_RPC_PORT ?? '9100', 10)
  ...
  platform.logger.info(`Depot headless server listening on ${wsServer.protocol}://${rpcHost}:${wsServer.port}`)
