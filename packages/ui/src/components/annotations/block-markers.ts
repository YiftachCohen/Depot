import type { AnnotationV1 } from '@depot/core'
...
export function clearBlockAnnotationMarkers(root: HTMLElement): void {
  ...
  blocks.forEach((block) => {
    ...
    block.style.boxShadow = '' // eslint-disable-line craft-styles/no-nonstandard-shadows
...
export function applyBlockAnnotationMarker(root: HTMLElement, annotation: AnnotationV1): void {
  ...
  target.style.boxShadow = 'inset 0 0 0 1px color-mix(in srgb, var(--info) 22%, transparent)' // eslint-disable-line craft-styles/no-nonstandard-shadows
}
