 ...
 * It uses the shared handlers from @depot/session-tools-core to ensure
...
import { isDeveloperFeedbackEnabled } from '@depot/shared/feature-flags';
...
// ============================================================
// Depot Agents Docs Upstream Proxy
// ============================================================
...
async function connectDocsUpstream(): Promise<void> {
  try {
    ...
    console.error(`Depot Agents Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`Depot Agents Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`Depot Agents Docs server is not connected. Tool '${name}' unavailable.`);
  }
