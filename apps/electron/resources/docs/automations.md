...
This guide explains how to configure automations in Depot Agent to automate workflows based on events.
...
Automations allow you to trigger actions automatically when specific events occur in Depot Agent. You can:
...
### App Events (triggered by Depot Agent)
...
Send a prompt to Depot Agent (creates a new session for scheduled prompts).
...
```json
{
  ...
  "method": "POST",
  "body": { "text": "Hello from Depot Agent!" }
}
```
...
Ask Depot Agent to validate your automations configuration:
...
2. **Check env vars** — Ensure `CRAFT_WH_*` variables are set in your shell profile and Depot Agent was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
