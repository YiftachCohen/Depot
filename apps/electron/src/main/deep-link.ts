 ...
 * Deep Link Handler
 *
 * Parses depot:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   depot://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   depot://flagged[/session/{sessionId}]             - Session list (flagged filter)
 *   depot://state/{stateId}[/session/{sessionId}]     - Session list (state filter)
 *   depot://sources[/source/{sourceSlug}]          - Sources list
 *   depot://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   depot://action/{actionName}[/{id}][?params]
 *   depot://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *
 * Examples:
 *   depot://allSessions                               (all sessions view)
 *   depot://allSessions/session/abc123                (specific session)
 *   depot://settings/shortcuts                     (shortcuts page)
 *   depot://sources/source/github                  (github source info)
 *   depot://action/new-chat                        (uses active window)
 *   depot://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   depot://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */
...
import { isSupportedDeepLinkProtocol } from './deep-link-scheme'
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // e.g., depot://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., depot://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // depot://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // depot://allSessions/..., depot://settings/..., etc. (compound routes)
    ...
    // depot://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // depot://action/... (no workspace - uses active window)
    if (host === 'action') {
