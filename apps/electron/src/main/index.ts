...
import { cleanupSessionFileWatchForClient } from '@depot/server-core/handlers/rpc'
import type { PlatformServices } from '../runtime/platform'
import type { HandlerDeps } from './handlers/handler-deps'
import type { RpcServer } from '@depot/server-core/transport'
import { WsRpcServer } from '../transport/server'
import { initModelRefreshService, getModelRefreshService, setFetcherPlatform } from '@depot/server-core/model-fetchers'
import { setSearchPlatform, setImageProcessor } from '@depot/server-core/services'
import { createApplicationMenu } from './menu'
import { WindowManager } from './window-manager'
import { loadWindowState, saveWindowState } from './window-state'
import { getWorkspaces, loadStoredConfig, addWorkspace, saveConfig } from '@depot/shared/config'
import { getDefaultWorkspacesDir } from '@depot/shared/workspaces'
import { initializeDocs } from '@depot/shared/docs'
import { initializeReleaseNotes } from '@depot/shared/release-notes'
import { ensureDefaultPermissions } from '@depot/shared/agent/permissions-config'
import { ensureToolIcons, ensurePresetThemes } from '@depot/shared/config'
import { setBundledAssetsRoot } from '@depot/shared/utils'
...
import { setPerfEnabled, enableDebug } from '@depot/shared/utils'
import { registerPiModelResolver } from '@depot/shared/config'
import { getPiModelsForAuthProvider, getAllPiModels } from '@depot/shared/config'
...
// These are available to all agent Bash sessions via DEPOT_UV, DEPOT_SCRIPTS env vars
// and PATH prepend. uv auto-downloads Python 3.12 on first use (~5s, then cached).
{
  ...
  process.env.DEPOT_IS_PACKAGED = app.isPackaged ? '1' : '0'
  process.env.DEPOT_RESOURCES_BASE = resourcesBase
  process.env.DEPOT_APP_ROOT = app.isPackaged ? app.getAppPath() : process.cwd()

  process.env.DEPOT_UV = bundledUvExists ? uvBinary : (fallbackUv ?? uvBinary)
  ...
  if (existsSync(bunBinary)) {
    process.env.DEPOT_BUN = bunBinary
  }

  process.env.DEPOT_SCRIPTS = scriptsDir
  process.env.DEPOT_COMMANDS_ENTRY = app.isPackaged
    ? join(app.getAppPath(), 'packages', 'depot-commands', 'src', 'main.ts')
    : join(process.cwd(), 'packages', 'depot-commands', 'src', 'main.ts')
  process.env.DEPOT_CLI_ENTRY = app.isPackaged
    ? join(app.getAppPath(), 'packages', 'depot-cli', 'src', 'cli.ts')
    : join(process.cwd(), 'packages', 'depot-cli', 'src', 'cli.ts')
  process.env.DEPOT_COMMANDS_DOC_PATH = app.isPackaged
    ? join(resourcesBase, 'resources', 'docs', 'depot-cli.md')
    : join(process.cwd(), 'apps', 'electron', 'resources', 'docs', 'depot-cli.md')
  process.env.DEPOT_CLI_DOC_PATH = process.env.DEPOT_COMMANDS_DOC_PATH
  process.env.DEPOT_AGENT_VERSION = app.getVersion()
  ...
  if (!bundledUvExists) {
    mainLog.warn('Bundled uv binary missing, CLI document tools may fail unless uv is available on PATH.', {
      ...
      usingDepotUv: process.env.DEPOT_UV,
    })
  }

  if (isDebugMode) {
    mainLog.info('CLI tools configured:', { uvBinary: process.env.DEPOT_UV, binDir, scriptsDir, bundledUvExists })
  }
}
...
// Only bypasses cert validation for the exact DEPOT_SERVER_URL origin — all other connections
...
if (process.env.DEPOT_SERVER_URL) {
  let serverOrigin: string | undefined
  try {
    serverOrigin = normalizeOriginForCert(process.env.DEPOT_SERVER_URL)
  } catch {
...
app.whenReady().then(async () => {
  ...
  process.env.DEPOT_IS_PACKAGED = app.isPackaged ? 'true' : 'false'
  ...
  // Seed tool icons to ~/.depot/tool-icons/ (copies bundled SVGs on first run)
  ensureToolIcons()

  // Seed preset themes to ~/.depot/themes/ (copies bundled theme JSONs on first run)
  ensurePresetThemes()
  ...
  if (process.platform === 'darwin' && app.dock) {
    ...
    // DEPOT_INSTANCE_NUMBER is set by detect-instance.sh for numbered folders
    const instanceNum = process.env.DEPOT_INSTANCE_NUMBER
    if (instanceNum) {
  ...
  try {
    ...
    // When DEPOT_SERVER_URL is set, this Electron instance is a thin client —
    ...
    const isClientOnly = !!process.env.DEPOT_SERVER_URL
    const isHeadless = !!process.env.DEPOT_HEADLESS

    if (isClientOnly) {
      mainLog.info(`Client-only mode: DEPOT_SERVER_URL=${process.env.DEPOT_SERVER_URL} (server initialization skipped)`)
    }
    ...
    if (!isClientOnly) {
      ...
      if (process.platform === 'win32') {
        const { getGitBashPath, clearGitBashPath } = await import('@depot/shared/config')
    ...
    if (!isClientOnly) {
      ...
      setSessionRuntimeHooks({
        ...
        captureException: (error, context) => {
          mainLog.error('[session] Exception:', error instanceof Error ? error.message : String(error), context)
        },
      })
    ...
    if (!isClientOnly) {
      ...
      // DEPOT_RPC_HOST / DEPOT_RPC_PORT allow binding to a custom address for remote access.
      const rpcHost = process.env.DEPOT_RPC_HOST ?? '127.0.0.1'
      const rpcPort = process.env.DEPOT_RPC_PORT ? parseInt(process.env.DEPOT_RPC_PORT, 10) : 0
      ...
      if (isHeadless) {
        console.log(`DEPOT_SERVER_URL=ws://${rpcHost}:${wsServer.port}`)
...
app.on('window-all-closed', () => {
  if (process.env.DEPOT_HEADLESS) return  // headless server stays alive
