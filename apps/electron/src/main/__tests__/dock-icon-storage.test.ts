...
mock.module('electron', () => ({
  nativeImage: {
    createFromBuffer: (buffer: Buffer) => ({
      isEmpty: () => buffer.toString('base64') !== VALID_PNG_BASE64,
    }),
  },
}))

const tempDirs: string[] = []
let storage: typeof import('../dock-icon-storage')
...
beforeAll(async () => {
  storage = await import('../dock-icon-storage')
})
...
describe('dock icon storage', () => {
  it('persists a validated PNG dock icon atomically', () => {
    ...
    const persistedPath = storage.persistDockIconDataUrl(VALID_PNG_DATA_URL, dockIconPath)
    ...
    expect(readFileSync(dockIconPath).subarray(0, 8).toString('hex')).toBe(
      storage.decodeValidatedDockIconDataUrl(VALID_PNG_DATA_URL).subarray(0, 8).toString('hex'),
    )
    expect(storage.hasValidPersistedDockIcon(dockIconPath)).toBe(true)
  })

  it('rejects non-PNG data URLs', () => {
    expect(() => storage.decodeValidatedDockIconDataUrl('data:image/svg+xml;base64,PHN2Zz48L3N2Zz4=')).toThrow(
  ...
  it('rejects oversized PNG payloads', () => {
    ...
    expect(() => storage.decodeValidatedDockIconDataUrl(oversizedDataUrl)).toThrow(
  ...
  it('falls back to bundled icon when the persisted icon is corrupt', () => {
    ...
    expect(storage.hasValidPersistedDockIcon(persistedDockIconPath)).toBe(false)
    expect(storage.resolveStartupDockIconPath([bundledDockIconPath], persistedDockIconPath)).toBe(bundledDockIconPath)
  })
