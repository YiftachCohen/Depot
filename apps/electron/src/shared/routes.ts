...
export const routes = {
  ...
  view: {
    ...
    /** Skills view (skills navigator). Pass a slug for agent detail, sessionId for agent session. */
    skills: (skillSlug?: string, sessionId?: string) => {
      if (!skillSlug) return 'skills' as const
      if (sessionId) return `skills/skill/${skillSlug}/session/${sessionId}` as const
