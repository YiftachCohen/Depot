...
// =============================================================================
export * from '@depot/shared/protocol'
...
import type {
  ...
  ToolDisplayMeta,
  AnnotationV1,
} from '@depot/core/types';

// Mode types from dedicated subpath export (avoids pulling in SDK)
import type { PermissionMode } from '@depot/shared/agent/modes';
export type { PermissionMode };
export { PERMISSION_MODE_CONFIG } from '@depot/shared/agent/modes';

// Thinking level types
import type { ThinkingLevel } from '@depot/shared/agent/thinking-levels';
export type { ThinkingLevel };
export { THINKING_LEVELS, DEFAULT_THINKING_LEVEL } from '@depot/shared/agent/thinking-levels';
...
import type { AuthState, SetupNeeds } from '@depot/shared/auth/types';
import type { AuthType } from '@depot/shared/config/types';
export type { AuthState, SetupNeeds, AuthType };

// Credential health types
import type { CredentialHealthStatus, CredentialHealthIssue, CredentialHealthIssueType } from '@depot/shared/credentials/types';
...
import type { LlmConnection, LlmConnectionWithStatus, LlmAuthType, LlmProviderType, NetworkProxySettings } from '@depot/shared/config';
...
import type { Workspace, SessionMetadata, StoredAttachment as StoredAttachmentType } from '@depot/core/types';
...
export interface ElectronAPI {
  ...
  getSourcePermissionsConfig(workspaceId: string, sourceSlug: string): Promise<import('@depot/shared/agent').PermissionsConfigFile | null>
  ...
  testSourceConnection(workspaceId: string, sourceSlug: string): Promise<{ success: boolean; error?: string; toolCount?: number }>
  ...
  // Skill-scoped session search (search across all sessions for a given skill)
  searchSessionsBySkill(workspaceId: string, skillSlug: string, query: string, limit?: number): Promise<SkillSearchResult[]>
  ...
  demoteAgent(workspaceId: string, skillSlug: string, skillPath?: string): Promise<void>
  ...
  listLabels(workspaceId: string): Promise<import('@depot/shared/labels').LabelConfig[]>
  createLabel(workspaceId: string, input: import('@depot/shared/labels').CreateLabelInput): Promise<import('@depot/shared/labels').LabelConfig>
  ...
  listViews(workspaceId: string): Promise<import('@depot/shared/views').ViewConfig[]>
  saveViews(workspaceId: string, views: import('@depot/shared/views').ViewConfig[]): Promise<void>
...
export interface SkillsNavigationState {
  navigator: 'skills'
  details: { type: 'skill'; skillSlug: string; sessionId?: string } | null
  rightSidebar?: RightSidebarPanel
}
  ...
  if (state.navigator === 'skills') {
    if (state.details?.type === 'skill') {
      if (state.details.sessionId) {
        return `skills/skill/${state.details.skillSlug}/session/${state.details.sessionId}`
      }
...
export const parseNavigationStateKey = (key: string): NavigationState | null => {
  ...
  if (key.startsWith('skills/skill/')) {
    const rest = key.slice(13) // after "skills/skill/"
    const sessionMatch = rest.match(/^(.+?)\/session\/(.+)$/)
    if (sessionMatch) {
      return { navigator: 'skills', details: { type: 'skill', skillSlug: sessionMatch[1], sessionId: sessionMatch[2] } }
    }
    if (rest) {
      return { navigator: 'skills', details: { type: 'skill', skillSlug: rest } }
    }
