...
export function parseCompoundRoute(route: string): ParsedCompoundRoute | null {
  ...
  if (first === 'skills') {
    ...
    // skills/skill/{skillSlug}/session/{sessionId} or skills/skill/{skillSlug}
    if (segments[1] === 'skill' && segments[2]) {
      const sessionId = (segments[3] === 'session' && segments[4]) ? segments[4] : undefined
      return {
        navigator: 'skills',
        details: { type: 'skill', id: segments[2], sessionId },
      }
    }
...
export function buildCompoundRoute(parsed: ParsedCompoundRoute): string {
  ...
  if (parsed.navigator === 'skills') {
    ...
    if (parsed.details.sessionId) return `skills/skill/${parsed.details.id}/session/${parsed.details.sessionId}`
...
function convertCompoundToNavigationState(compound: ParsedCompoundRoute): NavigationState {
  ...
  if (compound.navigator === 'skills') {
    ...
    return {
      navigator: 'skills',
      details: { type: 'skill', skillSlug: compound.details.id, sessionId: compound.details.sessionId },
    }
  }
...
function navigationStateToCompoundRoute(state: NavigationState): ParsedCompoundRoute {
  ...
  if (state.navigator === 'skills') {
    return {
      navigator: 'skills',
      details: state.details?.type === 'skill' ? { type: 'skill', id: state.details.skillSlug, sessionId: state.details.sessionId } : null,
    }
  }
