...
import { flattenLabels } from "@depot/shared/labels"
...
interface SessionListProps {
  ...
  /** Available skills for skill-based grouping and filtering */
  skills?: LoadedSkill[]
  /** Active skill filter slug (when set, only sessions with this skillSlug are shown) */
  skillFilter?: string | null
  /** Called when skill filter changes */
...
/**
 * SkillFilterBar - Horizontal pill bar for filtering sessions by skill.
 * Shows when skills exist and skill grouping/filtering is relevant.
 */
...
export function SessionList({
  ...
  const rowData = useMemo(() => {
    ...
    if (groupingMode === 'skill') {
      // Build a slug-to-name lookup from skills prop
      const skillNameMap = new Map<string, string>()
      for (const skill of skills) {
        skillNameMap.set(skill.slug, skill.metadata?.name || skill.slug)
      }

      // Build groups from visible items
      const groupsBySlug = new Map<string, { rows: SessionListRow[], slug: string }>()
      for (const row of rows) {
        const slug = row.item.skillSlug || '__ungrouped__'
        const key = `skill-${slug}`
        if (!groupsBySlug.has(key)) groupsBySlug.set(key, { rows: [], slug })
        groupsBySlug.get(key)!.rows.push(row)
      }

      // Insert collapsed placeholder groups
      for (const meta of collapsedGroupsMeta) {
        if (!groupsBySlug.has(meta.key)) {
          const slug = meta.key.replace('skill-', '')
          groupsBySlug.set(meta.key, { rows: [], slug })
        }
      }
      ...
      // Sort: named skills alphabetically, ungrouped last
      orderedGroups.sort((a, b) => {
        if (a.key === 'skill-__ungrouped__') return 1
        if (b.key === 'skill-__ungrouped__') return -1
        return a.label.localeCompare(b.label)
      })

      // If only one group exists, disable collapsing
      if (orderedGroups.length === 1) {
        orderedGroups[0].collapsible = false
      }
  ...
  }, [isSearchMode, matchingFilterItems, otherResultItems, flatItems, groupingMode, sessionStatuses, collapsedGroupsMeta, skills])
  ...
  const collapseAllGroups = useCallback(() => {
    ...
    } else if (groupingMode === 'skill') {
      const allKeys = new Set(items.map(item => `skill-${item.skillSlug || '__ungrouped__'}`))
      setCollapsedGroups(allKeys)
    } else {
  ...
  if (flatRows.length === 0 && rowData.groups.length === 0 && !searchActive) {
    ...
    if (currentFilter?.kind === 'archived') {
      return (
        <EntityListEmptyScreen
          ...
          description="Chats you archive will appear here. Archive chats to keep your list tidy while preserving conversations."
          className="h-full"
        />
      )
    }
    ...
    return (
      <EntityListEmptyScreen
        icon={<Inbox />}
        title="No chats yet"
        description="Chats with your agent appear here. Start one to get going."
        className="h-full"
      >
        <button
          onClick={() => {
