...
import type { LlmAuthType, LlmProviderType } from "@depot/shared/config/llm-connections"

/** Provider segment for the segmented control */
export type ProviderSegment = 'anthropic' | 'pi' | 'bedrock'
...
const SEGMENT_DESCRIPTIONS: Record<ProviderSegment, React.ReactNode> = {
  ...
  pi: <>Use Depot Backend as the main agent.<BetaBadge /><br />Configure with your API key, OAuth subscription, or custom endpoint.</>,
  bedrock: <>Use Claude via AWS Bedrock.<br />Configure with IAM credentials or environment variables.</>,
}
...
export function apiSetupMethodToConnectionTypes(method: ApiSetupMethod): {
  ...
  switch (method) {
    ...
    case 'bedrock_env':
      return { providerType: 'bedrock', authType: 'environment' };
  }
}
...
const API_SETUP_OPTIONS: ApiSetupOption[] = [
  ...
  },
  {
    id: 'pi_chatgpt_oauth',
    name: 'ChatGPT Plus',
    description: 'Use your ChatGPT subscription with Depot Backend.',
  ...
  },
  {
    id: 'pi_copilot_oauth',
    name: 'GitHub Copilot',
    description: 'Use your GitHub Copilot subscription with Depot Backend.',
    icon: <Cpu className="size-4" />,
    providerType: 'pi',
  },
  {
  ...
  },
  {
    ...
    description: 'Use AWS access keys to authenticate with Bedrock.',
    icon: <Key className="size-4" />,
    providerType: 'bedrock',
  },
  {
    ...
    description: 'Use AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY from your environment.',
    icon: <Cloud className="size-4" />,
    providerType: 'bedrock',
  },
]
...
function ProviderSegmentedControl({
  ...
  const segments: ProviderSegment[] = ['anthropic', 'pi', 'bedrock']
...
export function APISetupStep({
  ...
  const filteredOptions = API_SETUP_OPTIONS.filter(o =>
    activeSegment === 'bedrock'
      ? o.providerType === 'bedrock'
      : o.providerType === activeSegment
  )
