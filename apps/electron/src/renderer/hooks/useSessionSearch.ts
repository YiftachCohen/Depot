...
import { fuzzyScore } from "@depot/shared/search"
...
export interface UseSessionSearchOptions {
  ...
  groupingMode?: 'date' | 'status' | 'skill'
  /** Ref to the ScrollArea viewport element — used for scroll-based pagination */
  scrollViewportRef?: React.RefObject<HTMLDivElement>
  /** Skill filter — when set, only sessions with this skillSlug are shown */
...
function getCollapseGroupKey(item: SessionMeta, groupingMode?: 'date' | 'status' | 'skill'): string {
  if (groupingMode === 'status') return `status-${getSessionStatus(item)}`
  if (groupingMode === 'skill') return `skill-${item.skillSlug || '__ungrouped__'}`
...
export function useSessionSearch({
  ...
  const searchFilteredItems = useMemo(() => {
    ...
    if (skillFilter) {
      result = result.filter(item => item.skillSlug === skillFilter)
    }

    return result
  }, [sortedItems, isSearchMode, searchQuery, contentSearchResults, currentFilter, evaluateViews, statusFilter, labelFilterMap, skillFilter])
