...
import { AlertCircle, ArrowLeft, Globe, Copy, RefreshCw, Link2Off, Info, Zap } from 'lucide-react'
...
const ChatPage = React.memo(function ChatPage({ sessionId }: ChatPageProps) {
  ...
  // Resolve skill slug: prefer nav context, fall back to session metadata
  const effectiveSkillSlug = skillSlugFromNav ?? sessionMeta?.skillSlug ?? undefined

  // Resolve skill name from the skills list
  const activeSkill = React.useMemo(() => {
    if (!effectiveSkillSlug) return null
    return skills?.find(s => s.slug === effectiveSkillSlug) ?? null
  }, [skills, effectiveSkillSlug])

  const skillBadge = React.useMemo(() => {
    if (!activeSkill) return undefined
    return (
      <button
        onClick={() => navigate(routes.view.skills(activeSkill.slug))}
        className="inline-flex items-center gap-1 h-5 px-2 text-[10px] font-medium rounded bg-foreground/[0.06] text-muted-foreground hover:bg-foreground/[0.10] transition-colors cursor-pointer"
      >
        <Zap className="h-3 w-3" />
        {activeSkill.metadata.name || activeSkill.slug}
      </button>
    )
      ...
      return (
        <>
          <div className="h-full flex flex-col">
            <PanelHeader  title={displayTitle} titleMenu={titleMenu} badge={skillBadge} leftAction={backToAgentButton} actions={shareButton} rightSidebarButton={rightSidebarButton} isRegeneratingTitle={isAsyncOperationOngoing} />
    ...
    return (
      <div className="h-full flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-3 text-muted-foreground">
          ...
          <p className="text-sm">This chat no longer exists</p>
        </div>
      </div>
    )
  }
