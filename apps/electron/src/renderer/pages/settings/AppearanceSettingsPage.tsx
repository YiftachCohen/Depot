...
function AppIconPicker() {
  const activeId = useDepotIconId()

  return (
    <SettingsSection
      title="App Icon"
      description="Choose the icon shown in the toolbar, dock, and splash screen"
    >
      <SettingsCard>
        <div className="p-4">
          <div className="grid grid-cols-4 gap-3 sm:grid-cols-6">
            {DEPOT_ICONS.map((icon) => {
              const isActive = icon.id === activeId
              return (
                <button
                  key={icon.id}
                  type="button"
                  onClick={() => setDepotIcon(icon.id)}
                  className={`
                    relative flex flex-col items-center gap-1.5 rounded-xl p-2.5
                    transition-all duration-150 cursor-pointer
                    ${isActive
                      ? 'bg-accent/15 ring-2 ring-accent'
                      : 'bg-foreground/[0.03] hover:bg-foreground/[0.06] ring-1 ring-foreground/[0.06]'
                    }
                  `}
                >
                  {isActive && (
                  ...
                  <div className="w-[56px] h-[56px] flex items-center justify-center rounded-lg bg-[#0c0a1d]">
                    {icon.preview()}
                  </div>
                  <span className="text-[10px] text-foreground/60 leading-tight text-center truncate w-full">
                    {icon.name}
                  </span>
                </button>
              )
