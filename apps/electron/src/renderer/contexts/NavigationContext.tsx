...
import { parsePermissionMode } from '@depot/shared/agent/mode-types'
...
export function NavigationProvider({
  ...
  const [canGoBackHistory, setCanGoBackHistory] = useState(false)
  const [canGoForward, setCanGoForward] = useState(false)

  // canGoBack includes logical parent fallback: if browser history is empty
  // but the current route has a parent (e.g. session → agent → dashboard),
  // the back button should still be enabled.
  const hasLogicalParent = useMemo(() => {
    if (isSkillsNavigation(navigationState) && navigationState.details) return true
    if (isSessionsNavigation(navigationState) && navigationState.details) return true
    if (isSourcesNavigation(navigationState) && navigationState.details) return true
    return false
  }, [navigationState])
  const canGoBack = canGoBackHistory || hasLogicalParent
  ...
  const updateCanGoBackForward = useCallback(() => {
    setCanGoBackHistory(historySeqRef.current > 0)
  ...
  const reconcileFromUrlParams = useCallback(
    (params: URLSearchParams, options?: { skipAutoSelect?: boolean }) => {
      ...
      // When restoring from back/forward (skipAutoSelect), preserve routes
      // exactly as encoded in the URL — don't auto-select items for list views.
      // This prevents back from `skills/skill/x` → `allSessions` from
      // auto-selecting a session (which would show a chat instead of the dashboard).
      const resolveRoute = options?.skipAutoSelect
        ? (route: ViewRoute) => route
        : (route: ViewRoute) => normalizePanelRouteForReconcile(route, (state) => resolveAutoSelectionRef.current(state))
      ...
      if (panelsParam) {
        ...
        entries = panelsParam.split(',').filter(Boolean).map(entry => {
          ...
          if (colonIdx > 0) {
            ...
            if (!isNaN(proportion) && proportion > 0 && proportion < 1) {
              ...
              return { route: resolveRoute(rawRoute), proportion }
            }
          }
          const rawRoute = entry as ViewRoute
          return { route: resolveRoute(rawRoute), proportion: 0 }
        })
        ...
        if (navState) {
          const finalRoute = options?.skipAutoSelect
  ...
  const resolveAutoSelection = useCallback(
    (newState: NavigationState, options?: { skipAutoSelect?: boolean }): NavigationState => {
      ...
      // Skills: show dashboard (no auto-select — the SkillDashboard is the landing view)
  ...
  const goBack = useCallback(() => {
    ...
    // Logical parent fallback: navigate "up" in the route hierarchy
    // skills/skill/{slug}/session/{id} → skills/skill/{slug}
    // skills/skill/{slug} → skills
    // allSessions/session/{id} → allSessions
    if (isSkillsNavigation(navigationState) && navigationState.details) {
      if (navigationState.details.sessionId) {
        navigate(routes.view.skills(navigationState.details.skillSlug))
      } else {
  ...
  }, [canGoBack, navigationState, navigate])
  ...
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      ...
      // Skip auto-selection so back/forward restores the exact URL state
      // (prevents e.g. `allSessions` from auto-selecting a session on back).
      suppressPushRef.current = true
      reconcileFromUrlParamsRef.current(params, { skipAutoSelect: true })
  ...
  useEffect(() => {
    ...
    if (isPopstateSwitchRef.current) {
      ...
      reconcileFromUrlParamsRef.current(new URLSearchParams(window.location.search), { skipAutoSelect: true })
      lastSemanticHistoryKeyRef.current = getSemanticHistoryKey()
    } else {
  ...
  const navigateToSession = useCallback((sessionId: string) => {
    if (!isSessionsNavigation(navigationState)) {
      // If session belongs to an agent, navigate within agent context
